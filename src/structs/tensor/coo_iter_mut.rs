    ...
    VT: 'a + ValType,
{
    ...
    result_buffer: Option<(&'a [IT], &'a mut VT)>,
}
    ...
    fn calc_result(&mut self) -> (&'a [IT], &'a mut VT) {
        ...
        unsafe {
            (
                ...
                (result.1 as *const VT as *mut VT)
                    .as_mut::<'a>()
                    .unwrap_unchecked(),
            )
        }
...
impl<'a, IT, VT> StreamingIterator for COOIterMut<'a, IT, VT>
where
    IT: 'a + IdxType,
    VT: 'a + ValType,
{
...
impl<'a, IT, VT> DoubleEndedStreamingIterator for COOIterMut<'a, IT, VT>
where
    IT: 'a + IdxType,
    VT: 'a + ValType,
{
